...
func SyncPrices() error {
	...
	savedPrices, err := FetchPrices(context.TODO(), cfg.DB)
	if err != nil {
		return err
	}

	prices, err := loadPrices(cfg.DB)
	if err != nil {
		return err
	}

	return insertPrices(cfg.DB, savedPrices, prices)
}
...
func ValidatePrices() error {
	...
	sets, err := FetchSets(context.TODO(), cfg.DB)
	if err != nil {
		return err
	}
	for _, set := range sets {
		...
		_, err = associatePrices(cfg.DB, set, prices)
		if err != nil {
			return err
		}
	}
